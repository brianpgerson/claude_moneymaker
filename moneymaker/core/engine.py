    ...
    async def run_cycle(self) -> dict[str, Any]:
                ...
                if symbols:
                    ...
                    for symbol, price in prices.items():
                        if symbol in state.positions:
                            state.positions[symbol].current_price = price
                            # If no entry price, use current as entry (first sync)
                            if state.positions[symbol].average_entry_price == 0:
                                state.positions[symbol].average_entry_price = price
        ...
        if self.settings.trading_mode == TradingMode.LIVE and orders:
            ...
            if exchange_balances is not None:
                ...
                # Update position prices from filled orders
                for order in orders:
                    if order.status.value == "filled" and order.filled_price:
                        state = self.portfolio.get_state()
                        if order.symbol in state.positions:
                            pos = state.positions[order.symbol]
                            pos.current_price = order.filled_price
                            pos.average_entry_price = order.filled_price
